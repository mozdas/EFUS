...
def automated_window_lfp(main_path, tw, group_to_plot, trodes_to_plot, y_scale, normalization_t1, normalization_t2):
    ...
    for folder in (folder for folder in dirs if ((folder != 'log.txt') and (folder != 'notes.docx') and (folder != 'analysis_files') and (folder != 'other') and (folder != 'analyzed') and (folder != '.DS_Store')and (folder != '._.DS_Store') and (folder != 'analyzed'))):
        ...
        for window in range(num_window):
             ...
             if(len(trodes_to_plot)>1): 
                 ...
                 savefig(analyzed_path_for_group + 'tw_pdf_format/' + 'list/' + 'average_evoked_LFP_at_time_window_{:g}_for_list'.format(window)+'.pdf', format = 'pdf')
                 savefig(analyzed_path_for_group + 'tw_svg_format/' + 'list/' +  'average_evoked_LFP_at_time_window_{:g}_for_list'.format(window)+'.svg', format = 'svg')
                 close()
             ...
             for trode in trodes_to_plot:
                 ...
                 savefig(analyzed_path_for_group + 'tw_pdf_format/' + 'electrode_{:g}/'.format(trode) +'average_evoked_LFP_at_time_window_{:g}_for_group_{:g}_electrode_{:g}'.format(window, group, trode)+'.pdf', format = 'pdf')
                 savefig(analyzed_path_for_group + 'tw_svg_format/' + 'electrode_{:g}/'.format(trode) +'average_evoked_LFP_at_time_window_{:g}_for_group_{:g}_electrode_{:g}'.format(window, group, trode)+'.svg', format = 'svg')
                 close()
        ...
        for trode in range(p['nr_of_electrodes_per_group']):
            ...
            plot(windows, evoked_window_amps[probe][group][trode], 'k-')
            xlabel('Time (min)')
            ylabel('Peak voltage (uV)')
            ylim_min = np.floor(np.min(evoked) / 100) * y_scale
            ylim_max = np.ceil(np.max(evoked) / 100) * y_scale
            ...
            errorbar(windows, evoked_window_amps[probe][group][trode], yerr = evoked_window_peak_errs[probe][group][trode])
            ...
            plot(windows, evoked_window_amps_corrected[probe][group][trode], 'k-')
            xlabel('Time (min)')
            ylabel('Peak voltage (uV)')
            ylim_min = np.floor(np.min(evoked) / 100) * y_scale
            ylim_max = np.ceil(np.max(evoked) / 100) * y_scale
            ...
            axes().set_yticks(y_ticks, minor=True)
            axes().grid(which='both')
            print('Plotting the Corrected Automated Time Window Analysis / Electrode-{}\n'.format(trode))
            errorbar(windows, evoked_window_amps_corrected[probe][group][trode], yerr = evoked_window_peak_errs[probe][group][trode], color='olive')
        ...
        np.save(analyzed_path_for_folder +'/probe_{0}_group_{1}/{2}_{3}mins_tw_all_electrodes_{4}_lfp_averages_probe_{5}_group_{6}.npy'.format(probe,group,experiment_id,tw,folder,probe,group), lfp_averages[probe][group]) # save averages as npy folder, use combining_graphs script for combining the result of several recordings
        np.save(analyzed_path_for_folder +'/probe_{0}_group_{1}/{2}_{3}mins_tw_all_electrodes_{4}_evoked_window_peak_errs_probe_{5}_group_{6}.npy'.format(probe,group,experiment_id,tw,folder,probe,group), evoked_window_peak_errs[probe][group]) # save errors
        save_file = analyzed_path_for_folder + '/probe_{0}_group_{1}/experiment-{2}_probe_{3}_group_{4}_window_LFP.pickle'.format(probe,group,experiment_id,probe,group)
        pickle.dump({'evoked_window_avgs':evoked_window_avgs, 'evoked_window_err':evoked_window_err}, open(save_file, 'wb'), protocol = -1)

        #Save the window LFP analysis of the user's list of electrodes
        electrode_names = '-'.join(str(trode) for trode in trodes_to_plot)
        np.save(analyzed_path_for_folder +'/probe_{0}_group_{1}/{2}_{3}mins_tw_{4}_{5}_list_lfp_averages_probe_{6}_group_{7}.npy'.format(probe,group,experiment_id,tw,electrode_names,folder,probe,group), list_lfp_averages[probe][group])
        np.save(analyzed_path_for_folder +'/probe_{0}_group_{1}/{2}_{3}mins_tw_{4}_{5}_list_peaks_errs_combined_probe_{6}_group_{7}.npy'.format(probe,group,experiment_id,tw,electrode_names,folder,probe,group), list_peaks_errs_combined[probe][group]) 
        save_file = analyzed_path_for_folder + '/probe_{:g}_group_{:g}/probe_{:g}_group_{:g}_list_window_LFP.pickle'.format(probe,group,probe,group)
        pickle.dump({'list_evoked_window_avg':list_evoked_window_avg, 'list_window_errs_combined':list_window_errs_combined}, open(save_file, 'wb'), protocol = -1)
