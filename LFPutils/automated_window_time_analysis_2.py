...
#tw = 1
#group_to_plot =0
#trodes_to_plot = [7,8,9] 
#y_scale = 50 
#normalization_t1 = 5 
#normalization_t2 = 15
#FUS_on_time = 10
#number=2 ##number of desired electrodes
#rolling_window=5
    
    ########################## 

    #Finding the electrodes with biggest responses
def automated_window_analysis(main_path, tw, y_scale, normalization_t1, normalization_t2, FUS_on_time, number,rolling_window):
    ...
    for folder in (folder for folder in dirs if ((folder != 'log.txt') and (folder != 'notes.docx') and (folder != 'analysis_files') and (folder != 'other') and (folder != 'analyzed') and (folder != '.DS_Store'))):
        if(folder==doc):        
            p = pickle.load(open(main_path + folder + '/paramsDict.p', 'rb'))
            
            peak_info = {key: {} for key in range(p['probes'])}
            evoked_avg_first=[None]*p['nr_of_electrodes_per_group']
            biggest_response=[[None]*number,[None]*number]
            for probe in range(p['probes']):
                ...
                for group in range(p['nr_of_groups']):
                    ...
                    data = pickle.load(open(main_path + folder + '/probe_{:g}_group_{:g}/probe_{:g}_group_{:g}_evoked.pickle'.format(probe,group,probe,group), 'rb'))
                    ...
                    evoked_min=np.zeros((p['nr_of_electrodes_per_group']))
        ...
        #			time = np.linspace(-p['evoked_pre']*1000, p['evoked_post']*1000, (p['evoked_post'] + p['evoked_pre']) * (p['sample_rate']/30))
                    ...
                    for trode in range(p['nr_of_electrodes_per_group']):
                        evoked_min[trode]= min(evoked_avg[trode])
			        
                    for trode in range(p['nr_of_electrodes_per_group']): 
                        evoked_avg_first[trode]=0
                        mm=0   
                        for tim in range(len(time)):
                            if (time[tim]<=0): 
                                evoked_avg_first[trode]=evoked_avg_first[trode]+evoked_avg[trode][tim] 
                                mm=mm+1
                        evoked_avg_first[trode]=evoked_avg_first[trode]/mm 
			            
                    evoked_min=evoked_min-evoked_avg_first   
                    deneme=np.append(deneme,evoked_min)
        	
            smallest=nsmallest(number,deneme)
            for numb in range(number):
                indices=np.append(indices,np.where(deneme==smallest[numb]))
            
            for numb in range(number):    
                if (indices[numb]<=15):
                    biggest_response[1][numb]=int(indices[numb])
                    biggest_response[0][numb]=0
                else:
                    biggest_response[1][numb]=int(indices[numb]-16)
                    biggest_response[0][numb]=1
            print('The electrodes with biggest responses are:')                
            print(biggest_response)

    ###### Automated Time Window Analysis starts from here:
    ...
    def automated_window_time(main_path, tw, group_to_plot, trodes_to_plot, y_scale, normalization_t1, normalization_t2,FUS_on_time,rolling_window):
    ...
    c_1=0
    which=[None]*number
    for trode in range(len(biggest_response[0])):
        if (biggest_response[0][trode]==0):
            group_0=group_0+1
        else:
            group_1=group_1+1
    
    trodes_0_to_plot=[None]*group_0
    trodes_1_to_plot=[None]*group_1
    for trode in range(number):
        if (biggest_response[0][trode]==0):
            trodes_0_to_plot[group_0-1]=biggest_response[1][trode]
            group_0=group_0-1 
            c_0=c_0+1 
        elif(biggest_response[0][trode]==1):
            trodes_1_to_plot[group_1-1]=biggest_response[1][trode]
            group_1=group_1-1
            c_1=c_1+1
    trodes_0_to_plot=sorted(trodes_0_to_plot)   
    trodes_1_to_plot=sorted(trodes_1_to_plot)    
    
    if (c_0!=0):
        automated_window_time(main_path, tw, 0, trodes_0_to_plot, y_scale, normalization_t1, normalization_t2, FUS_on_time,rolling_window)
    if (c_1!=0):
        automated_window_time(main_path, tw, 1, trodes_1_to_plot, y_scale, normalization_t1, normalization_t2, FUS_on_time,rolling_window)

#automated_window_analysis(main_path, tw, y_scale, normalization_t1, normalization_t2, FUS_on_time, number,rolling_window)
