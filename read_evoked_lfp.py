...
def read_evoked_lfp(probe,group,p,data):
 ...
 #   pickle.dump({'evoked':evoked_downsampled, 'stim_timestamps':stim_downsampled}, open(save_file, 'wb'), protocol=-1)
    ...
    if (p['fourier_transform'] == True):
        #amplitude = fft.fft(filtered, axis=1)
        #x = fft.fftfreq(len(evoked[0][0])*len(evoked[0]), p['sample_rate'])
        #frequncy = np.linspace(0.0, 1/p['sample_frequency'], data.shape[1])

        nfft = 10000
        #w = signal.get_window('boxcar',10000)
        #print(w.shape)
        f, t, s = signal.spectrogram(np.mean(evoked, axis=0), fs=p['sample_rate'], axis=1, nperseg=128, nfft=512, noverlap=30)
        #f, t, s = signal.spectrogram(np.mean(evoked, axis=0), fs=p['sample_rate'], axis=1, window=w, noverlap=9000, nperseg=10000)
        print(evoked.shape)
        print(s.shape)
        print(s)
        fid = open( '{0}/probe_{1}_group_{2}/probe_{3}_group_{4}_fourier_transformed_evoked.pickle'.format(p['path'],probe,group,probe,group),'wb')
        pickle.dump({'amplitude':s, 'frequency':f, 'time':t}, fid, protocol=-1)
        fid.close()
...
"""
    if (p['fourier_transform'] == True):
        ...
        print('\n')
        print(evoked.shape)
        print(np.swapaxes(evoked,0,1)[:].reshape(16,-1).shape)
        yr = np.mean(fft.fft(evoked, axis=2), axis=1) # "raw" FFT with both + and - frequencies
        x = np.linspace(0.0, p['sample_rate'], len(yr[0]))
        fid = open( '{0}/probe_{1}_group_{2}/probe_{3}_group_{4}_fourier_transformed_evoked.pickle'.format(p['path'],probe,group,probe,group),'wb')
        pickle.dump({'amplitude':yr, 'frequency':x}, fid, protocol=-1)
        fid.close()
    ...
    if (p['fourier_transform'] == True):
        ...
        #frequncy = np.linspace(0.0, 1/p['sample_frequency'], data.shape[1])
        print('\n')
        x, yr = signal.periodogram(evoked.reshape(16,-1), p['sample_rate'])
        fid = open( '{0}/probe_{1}_group_{2}/probe_{3}_group_{4}_fourier_transformed_periodogram.pickle'.format(p['path'],probe,group,probe,group),'wb')
        pickle.dump({'amplitude':yr, 'frequency':x}, fid, protocol=-1)
        fid.close()
    ...
    if (p['fourier_transform'] == True):
        ...
        #x = fft.fftfreq(len(evoked[0][0])*len(evoked[0]), p['sample_rate'])
        #frequncy = np.linspace(0.0, 1/p['sample_frequency'], data.shape[1])
        yr = fft.fft(data, axis=1) # "raw" FFT with both + and - frequencies
        x = np.linspace(0.0, p['sample_rate'], len(data[0]))
        fid = open( '{0}/probe_{1}_group_{2}/probe_{3}_group_{4}_fourier_transformed_raw.pickle'.format(p['path'],probe,group,probe,group),'wb')
        pickle.dump({'amplitude':yr, 'frequency':x}, fid, protocol=-1)
        fid.close()
    ...
    if (p['fourier_transform'] == True):
        ...
        #x = fft.fftfreq(len(evoked[0][0])*len(evoked[0]), p['sample_rate'])
        #frequncy = np.linspace(0.0, 1/p['sample_frequency'], data.shape[1])
        yr = fft.fft(filtered, axis=1) # "raw" FFT with both + and - frequencies
        x = np.linspace(0.0, p['sample_rate'], len(filtered[0]))
        fid = open( '{0}/probe_{1}_group_{2}/probe_{3}_group_{4}_fourier_transformed_filtered.pickle'.format(p['path'],probe,group,probe,group),'wb')    
        pickle.dump({'amplitude':yr, 'frequency':x}, fid, protocol=-1)
        fid.close()
